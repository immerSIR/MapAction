...
export default function CitizenTable() {
    ...
    // Modal state management with useDisclosure
    const {
    ...
    const fetchUserData = async () => {
        try {
            ...
            let donne = response.data.results.filter(
    ...
    const addUser = async (e) => {
        ...
        try {
            ...
            setInProgress(false);
            onNewUserModalClose(); // Correct closing of modal
            setNewUser({
                first_name: "",
                last_name: "",
                email: "",
    ...
    const onUpdateUser = async (e) => {
        ...
        try {
            ...
            setData((prevData) =>
                prevData.map((user) =>
                    user.id === newUser.id ? response.data : user
                )
            );
            setInProgress(false);
            onEditUserModalClose(); // Correct closing of modal
            setNewUser({
                id: "",
                first_name: "",
                last_name: "",
    ...
    const renderEditUserModal = () => (
        <Modal isOpen={isEditUserModalOpen} onClose={onEditUserModalClose}>
            ...
            <ModalContent>
                ...
                <ModalCloseButton />
                <ModalBody>
                    {/* Similar content as the New User modal */}
                </ModalBody>
                <ModalFooter>
                    <Button
                        colorScheme="blue"
    ...
    const onDeleteUser = (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .delete(`${config.url}/MapApi/user/${item.id}/`, {
                        headers: {
                            Authorization: `Bearer ${sessionStorage.token}`,
                        },
                    })
